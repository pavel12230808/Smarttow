...
public class GetPaidFinesActivity extends AppCompatActivity {
    ListView list_view;
    ...
    public void serverData(){
        progressDialog = new ProgressDialog(GetPaidFinesActivity.this);
        progressDialog.setMessage("Loading....");
        progressDialog.show();

        EndPointUrl service = RetrofitInstance.getRetrofitInstance().create(EndPointUrl.class);
        Call<List<PaymentDuePojo>> call = service.getPaidFines(session);
        call.enqueue(new Callback<List<PaymentDuePojo>>() {
            ...
            public void onResponse(Call<List<PaymentDuePojo>> call, Response<List<PaymentDuePojo>> response) {
                //Toast.makeText(GetPaidFinesActivity.this,"AAA   "+response.body().size(),Toast.LENGTH_SHORT).show();

                progressDialog.dismiss();
                if(response.body()==null){
                    Toast.makeText(GetPaidFinesActivity.this,"No data found",Toast.LENGTH_SHORT).show();
                }else {
                    a1 = response.body();
                    list_view.setAdapter(new GetPaidFinesAdapter(a1, GetPaidFinesActivity.this));
            ...
            public void onFailure(Call<List<PaymentDuePojo>> call, Throwable t) {
                progressDialog.dismiss();
                Toast.makeText(GetPaidFinesActivity.this, "Something went wrong...Please try later!", Toast.LENGTH_SHORT).show();
            }
        });
    }
    @Override
