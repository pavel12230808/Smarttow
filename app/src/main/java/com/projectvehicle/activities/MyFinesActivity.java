...
public class MyFinesActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        getSupportActionBar().setTitle("My Fines List");
    ...
    public void serverData(){
        progressDialog = new ProgressDialog(MyFinesActivity.this);
        progressDialog.setMessage("Loading....");
        progressDialog.show();

        EndPointUrl service = RetrofitInstance.getRetrofitInstance().create(EndPointUrl.class);
        Call<List<PaymentDuePojo>> call = service.getMyFines(session);
        call.enqueue(new Callback<List<PaymentDuePojo>>() {
            ...
            public void onResponse(Call<List<PaymentDuePojo>> call, Response<List<PaymentDuePojo>> response) {
                ...
                if(response.body()==null){
                    Toast.makeText(MyFinesActivity.this,"No data found",Toast.LENGTH_SHORT).show();
                } else if (response.body().size()==0) {
                    Toast.makeText(MyFinesActivity.this,"No Pending Payments",Toast.LENGTH_SHORT).show();

                } else {
                    a1 = response.body();
                    list_view.setAdapter(new GetAllPendingFinesAdapter(a1, MyFinesActivity.this));
            ...
            public void onFailure(Call<List<PaymentDuePojo>> call, Throwable t) {
                progressDialog.dismiss();
                Toast.makeText(MyFinesActivity.this, "Something went wrong...Please try later!", Toast.LENGTH_SHORT).show();
            }
        });
    }
    @Override
